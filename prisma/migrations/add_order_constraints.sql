...
-- ============================================================================
-- CONSTRAINT 3: Positive amount constraint for orders.total_amount
-- ============================================================================
...
-- ============================================================================
-- CONSTRAINT 4: Positive amount constraints for production_orders
-- ============================================================================
...
BEGIN
  ...
  IF constraint_count = 5 THEN
    RAISE NOTICE '';
    RAISE NOTICE '=================================================================';
    RAISE NOTICE '✓ SUCCESS: All 5 constraints added successfully';
    RAISE NOTICE '=================================================================';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '  4. production_orders_total_cost_positive';
    RAISE NOTICE '  5. production_orders_deposit_amount_positive';
    RAISE NOTICE '';
  ELSE
    RAISE EXCEPTION '✗ FAILED: Expected 5 constraints, found %', constraint_count;
  END IF;
END $$;
...
-- INSERT INTO orders (order_number, customer_id, status) VALUES ('INVALID', (SELECT id FROM customers LIMIT 1), 'pending');
-- Expected: ERROR - violates check constraint "orders_number_format"

-- Test 3: Try to create order with negative amount (should FAIL)
-- INSERT INTO orders (order_number, customer_id, total_amount, status) VALUES ('ORD-999999', (SELECT id FROM customers LIMIT 1), -100.00, 'pending');
