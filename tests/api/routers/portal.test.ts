...
if (!isDatabaseAvailable) {
  if (IS_CI) {
    console.warn('⚠️  Portal API Router tests skipped in CI environment');
    console.warn('   These tests require a live database connection');
  } else {
    console.warn('⚠️  Portal API Router tests skipped: DATABASE_URL not available');
  }
}

// Skip entire suite if no database available (e.g., in CI without TEST_DATABASE_URL)
describe.skipIf(!isDatabaseAvailable)('Portal API Router Tests', () => {
