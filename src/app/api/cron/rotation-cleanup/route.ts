...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!cronSecret) {
      log.error('[Cron:Rotation] CRON_SECRET not configured');
    ...
    if (authHeader !== `Bearer ${cronSecret}`) {
      log.error('[Cron:Rotation] Unauthorized request - invalid secret');
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 401 }
      );
    ...
    log.info('[Cron:Rotation] Starting rotation cleanup job...');

    // Run rotation cleanup job
    const result = await runRotationCleanupJob();

    log.info('[Cron:Rotation] Job completed:', {
    ...
    log.error('[Cron:Rotation] Error:', { error });
    return NextResponse.json(
      {
        success: false,
