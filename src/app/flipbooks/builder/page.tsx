    ...
    if (result?.pdfUrl && uploadType === "pdf") {
      ...
      try {
        ...
        if (extractionResult.success && extractionResult.itemCount > 0) {
          // PDF has bookmarks - use extracted TOC
          tocData = {
          ...
          // No bookmarks - create placeholder TOC for all pages
          const pageCount = result.pagesExtracted || 0;
          const placeholderItems = Array.from({ length: pageCount }, (_, i) => ({
        ...
        toast.success(
          `Table of Contents generated with ${itemCount} items!`,
          {
            description: itemCount === (result.pagesExtracted || 0)
              ? "Placeholder TOC created. Edit titles in the TOC tab."
              : "You can edit it in the TOC tab.",
            duration: 5000,
          }
        );
      } catch (error) {
