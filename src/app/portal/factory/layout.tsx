...
/**
 * Factory Portal Layout
 * Phase 4: Partner portal for factories
...
export default function FactoryPortalLayout({ children }: LayoutProps) {
  ...
  if (pathname === '/portal/factory/login' || pathname === '/portal/login') {
    return <>{children}</>;
  }
  ...
  const navigation = [
    { name: 'Dashboard', href: '/portal/factory', icon: LayoutDashboard, show: true },
    { name: 'Production Orders', href: '/portal/factory/orders', icon: Package, show: portalSettings?.modules?.orders !== false },
    { name: 'Shipping', href: '/portal/factory/shipping', icon: Truck, show: portalSettings?.modules?.shipping !== false },
    { name: 'Documents', href: '/portal/factory/documents', icon: FileText, show: portalSettings?.modules?.documents !== false },
    { name: 'Quality', href: '/portal/factory/quality', icon: CheckCircle, show: portalSettings?.modules?.quality !== false },
    { name: 'Settings', href: '/portal/factory/settings', icon: Settings, show: true },
            ...
            <div className="flex items-center space-x-3 mb-3">
              <div className="w-10 h-10 bg-[#91bdbd] rounded-full flex items-center justify-center">
                <span className="text-foreground text-sm font-medium">
                  {displayEmail[0]?.toUpperCase() || 'L'}
                </span>
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate">{displayEmail}</div>
                <div className="text-xs text-secondary">Factory Portal</div>
              </div>
            </div>
            <Button
              variant="outline"
