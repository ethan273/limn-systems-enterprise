...
// Predefined service templates for easy setup
const SERVICE_TEMPLATES = [
  {
    ...
    rotationDays: 180,
    description: 'QuickBooks Online API credentials for accounting integration',
  },
  {
  ...
  },
  {
    value: 'google_drive',
    label: 'Google Drive',
    fields: ['client_id', 'client_secret', 'folder_id'],
    rotationDays: 365,
    description: 'Google Drive API for large file storage',
  },
  {
    value: 'stripe',
...
export default function ApiKeysPage() {
  ...
  const handleCreate = async (data: Record<string, unknown>) => {
    const template = SERVICE_TEMPLATES.find((t) => t.value === data.service_template);
  ...
  const getCreateFormFields = (): FormField[] => {
    const template = SERVICE_TEMPLATES.find((t) => t.value === selectedService);
