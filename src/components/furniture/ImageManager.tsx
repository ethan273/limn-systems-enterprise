...
export function ImageManager({
 ...
 <div
 key={image.id}
 className="relative group card rounded-lg overflow-hidden border border hover:border transition-colors"
 >
 {/* Image Preview */}
 <div className="aspect-square card flex items-center justify-center relative">
 {image.file_url ? (
 <Image
 src={image.file_url}
 ...
 <div className="hidden absolute inset-0 flex items-center justify-center card">
 <config.icon className="h-8 w-8 text-tertiary" />
 </div>
 ...
 {image.description && (
 <div className="text-xs text-tertiary truncate mt-1">
 {image.description}
 </div>
 )}
 ...
 return (
 <Card key={imageType} className="card border">
 ...
 <p className="text-sm text-tertiary">{config.description}</p>
 </CardHeader>

 <CardContent>
 {typeImages.length === 0 ? (
 <div className="border-2 border-dashed border rounded-lg p-8 text-center">
 <config.icon className="h-12 w-12 text-secondary mx-auto mb-4" />
 <p className="text-tertiary mb-4">No {config.label.toLowerCase()} uploaded</p>
 {canUpload && (
 <Button
 type="button"
 variant="outline"
 ...
 <CardHeader>
 <CardTitle className="text-lg font-semibold ">Uploading...</CardTitle>
 </CardHeader>
 <CardContent>
 ...
 <div className="max-h-96 overflow-hidden rounded-lg card">
 <Image
