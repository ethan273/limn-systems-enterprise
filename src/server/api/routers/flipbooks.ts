...
export const flipbooksRouter = createTRPCRouter({
   ...
   * Extracts table of contents from the PDF file using PDF.js
   */
  generateTOC: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!flipbook.pdf_source_url) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Flipbook does not have a PDF source URL",
        });
      }

      // Extract TOC from PDF
      const result = await extractTOCFromPDF(flipbook.pdf_source_url);

      if (!result.success) {
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: result.errors?.[0] || "Failed to extract TOC from PDF",
        });
      }
